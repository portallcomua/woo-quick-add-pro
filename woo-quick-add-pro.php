 ...
 * Author: WooQuick
 * GitHub Plugin URI: portallcomua/woo-quick-add-pro
 * GitHub Branch: main
 */
    ...
    <div class="wrap" style="max-width:600px; margin:auto; padding:20px;">
        ...
        <?php else: ?>
            ...
            <div style="background:#e8f0fe; padding:20px; border-radius:10px;">
                ...
                <p>✅ Одноразова оплата, вічна ліцензія на 1 домен</p>
                <p>🔗 <a href="<?php echo WQA_SHOP_URL; ?>" target="_blank" style="background:#4CAF50; color:#fff; padding:10px 20px; text-decoration:none; border-radius:5px;">📦 ПЕРЕЙТИ ДО ОПЛАТИ</a></p>
                <hr>
                <form method="post">
                    ...
                    <button type="submit" name="wqa_activate_license" style="background:#2196F3; color:#fff; padding:10px 20px;">🔑 Активувати</button>
                </form>
...
function wqa_render_gui() {
    ...
    <div class="wrap" style="max-width: 700px; margin: auto; padding: 20px; font-family: sans-serif;">
        ...
        <div style="display: flex; gap: 10px; margin-bottom: 20px;">
            <a href="https://uaserver.pp.ua/readme_woo-quick-add" target="_blank" style="flex:1; background:#f0f0f0; border:1px solid #ccc; border-radius:10px; padding:10px; text-align:center; text-decoration:none;">📖 ІНСТРУКЦІЯ</a>
        ...
        <style>.photo-preview { width: 80px; height: 80px; object-fit: cover; border-radius: 10px; margin: 2px; border: 2px solid #ddd; }</style>
        ...
        document.getElementById('save_btn').onclick = async function() {
            this.disabled = true; this.innerText = "⏳ ЗБЕРІГАЮ...";
            const fd = new FormData();
            fd.append('action', 'wqa_save_all');
            ...
            try {
                ...
                if (res.success) {
                    document.getElementById('message').innerHTML = '✅ ТОВАР ДОДАНО! ID: ' + res.data.id;
                    ...
                    this.disabled = false; this.innerText = "➕ ОПУБЛІКУВАТИ";
                }
            } catch(e) {
...
add_action('wp_ajax_wqa_save_all', function() {
    if (!current_user_can('manage_options')) { wp_send_json_error('Немає прав'); return; }
    if (!wqa_can_add_product()) { wp_send_json_error('Ліміт вичерпано. <a href="' . admin_url('admin.php?page=wqa_license') . '">Придбайте ліцензію</a>'); return; }
    ...
    foreach ($lines as $line) {
        ...
        $val = trim($v);
        if (in_array($key, ['тег', 'мітка', 'позначка'])) { $tags[] = $val; }
        elseif ($key === 'атрибут') { $parts = explode(' ', $val, 2); if (count($parts) == 2) { $attrs[$parts[0]][] = $parts[1]; } }
        else { $data[$key] = $val; }
    }
    ...
    if (isset($data['категорія'])) { $term = term_exists($data['категорія'], 'product_cat'); if (!$term) $term = wp_insert_term($data['категорія'], 'product_cat'); if (!is_wp_error($term)) wp_set_object_terms($pid, [(int)$term['term_id']], 'product_cat'); }
    if (isset($data['бренд'])) { $tax = taxonomy_exists('pwb-brand') ? 'pwb-brand' : 'product_brand'; $term = term_exists($data['бренд'], $tax); if (!$term) $term = wp_insert_term($data['бренд'], $tax); if (!is_wp_error($term)) wp_set_object_terms($pid, [(int)$term['term_id']], $tax); }
    if (!empty($tags)) { $tag_ids = []; foreach ($tags as $tag) { if (empty($tag)) continue; $term = term_exists($tag, 'product_tag'); if (!$term) $term = wp_insert_term($tag, 'product_tag'); if (!is_wp_error($term)) $tag_ids[] = (int)$term['term_id']; } if (!empty($tag_ids)) wp_set_object_terms($pid, $tag_ids, 'product_tag'); }
    
    if (!empty($attrs)) { $product_attrs = []; foreach ($attrs as $name => $values) { $slug = sanitize_title($name); if (!wc_attribute_taxonomy_id_by_name($name)) { wc_create_attribute(['name' => $name, 'slug' => $slug, 'type' => 'select']); delete_transient('wc_attribute_taxonomies'); } $tax_name = wc_attribute_taxonomy_name($name); if (!taxonomy_exists($tax_name)) { register_taxonomy($tax_name, 'product', ['label' => $name, 'public' => false]); } $term_ids = []; foreach ($values as $val) { if (empty($val)) continue; $term = term_exists($val, $tax_name); if (!$term) $term = wp_insert_term($val, $tax_name); if (!is_wp_error($term)) $term_ids[] = (int)$term['term_id']; } if (!empty($term_ids)) wp_set_object_terms($pid, $term_ids, $tax_name); $product_attrs[$tax_name] = ['name' => $tax_name, 'value' => implode(', ', $values), 'is_visible' => 1, 'is_taxonomy' => 1]; } if (!empty($product_attrs)) update_post_meta($pid, '_product_attributes', $product_attrs); }
    
    $imgs = json_decode(stripslashes($_POST['images']), true);
    if (!empty($imgs)) { $gallery_ids = []; foreach ($imgs as $idx => $base64) { if (strpos($base64, 'data:image') === 0) { $imgData = base64_decode(preg_replace('#^data:image/\w+;base64,#i', '', $base64)); $filename = 'wqa_product_' . $pid . '_' . time() . '_' . $idx . '.jpg'; $upload = wp_upload_bits($filename, null, $imgData); if (!$upload['error']) { $attach_id = wp_insert_attachment(['post_mime_type' => 'image/jpeg', 'post_title' => 'Product Image', 'post_status' => 'inherit'], $upload['file'], $pid); require_once(ABSPATH . 'wp-admin/includes/image.php'); wp_update_attachment_metadata($attach_id, wp_generate_attachment_metadata($attach_id, $upload['file'])); if ($idx === 0) { set_post_thumbnail($pid, $attach_id); } else { $gallery_ids[] = $attach_id; } } } } if (!empty($gallery_ids)) update_post_meta($pid, '_product_image_gallery', implode(',', $gallery_ids)); }
